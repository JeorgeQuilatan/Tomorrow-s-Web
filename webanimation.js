var animations = {};
animations.leftFoot = document
    .querySelector(".foot.left")
    .animate([{ transform: "scale(1)" }, { transform: "scale(.6)" }], {
        duration: 700,
        iterations: Infinity,
        direction: "alternate",
        delay: 0 //100 * i
    });
animations.rightFoot = document
    .querySelector(".foot.right")
    .animate([{ transform: "scale(.6)" }, { transform: "scale(1)" }], {
        duration: 700,
        iterations: Infinity,
        direction: "alternate",
        delay: 0
    });
animations.rightHand = document
...
animations.body = document
    .querySelector(".body")
    .animate(
        [{ transform: "translateY(0)" }, { transform: "translateY(1rem)" }],
        {
            duration: 350,
            iterations: Infinity,
            direction: "alternate",
...
function pauseHandler(e) {
    ...
    if (player.playState === "paused") {
        ...
        document.querySelector(
            '[data-mapping="' + mapping + '"] input'
        ).disabled = true;
    } else {
        let currentRange = document.querySelector(
            '[data-mapping="' + mapping + '"] input'
        );
